...
import { Download, Upload, Plus, Edit, Trash, FileText, Upload as UploadIcon } from "lucide-react";
...
export default function Finance() {
  ...
  const handleAccountsTemplateUpload = async (event) => {
    ...
    try {
      ...
      loadFinanceData(); // Refresh accounts
    } catch (error) {
  ...
  const handleCashFlowsTemplateUpload = async (event) => {
    ...
    try {
      ...
      loadJournalEntries(); // Refresh journal entries
    } catch (error) {
